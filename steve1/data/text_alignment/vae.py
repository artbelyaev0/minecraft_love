...
def load_vae_model(vae_info):
    ...
    model = TranslatorVAE(input_dim=mineclip_dim, hidden_dim=hidden_dim, latent_dim=latent_dim, device=device)
...
class TranslatorVAE(torch.nn.Module):
    ...
    def __init__(self, input_dim=512, hidden_dim=256, latent_dim=256, device="cuda"):
        super().__init__()
        ...
        # self.bias = torch.tensor(np.load("/home/user21/STEVE-1/bias.npy")).to(device)
        # self.scalar = torch.tensor(np.load("/home/user21/STEVE-1/scalar.npy")).to(device)
