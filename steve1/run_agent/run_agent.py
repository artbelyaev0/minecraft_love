...
def run_agent(prompt_embed, gameplay_length, save_video_filepath,
    ...
    # time_for_one_action = math.ceil(gameplay_length / prompt_embed.shape[0])
    # print("shape", prompt_embed.shape, time_for_one_action)
    for _ in tqdm(range(gameplay_length)):
        # prompt_embed_now = prompt_embed[_ // time_for_one_action, :]
...
if __name__ == '__main__':
    ...
    if args.custom_text_prompt is not None:
        ...
        # texts_prompts = args.custom_text_prompt.split(", ")
        # print(f"prompt transpose to", texts_prompts)
        # prompt_embed = get_prior_embed(texts_prompts, mineclip, prior, DEVICE)
